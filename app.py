...
# 2. سیستەمێ هەلبژارتنا زمانی
if 'language' not in st.session_state:
    st.session_state.language = None

# 3. ژمارەکەرێ نهێنی (تنێ بۆ تە)
if 'visits' not in st.session_state:
    st.session_state.visits = 1200 
...
with st.sidebar:
    st.markdown("<h3 style='color:white;'>پشکا خودانی</h3>", unsafe_allow_html=True)
    pw = st.text_input("پاسۆرد بنڤیسە:", type="password")
    if pw == "matin2026":
        st.metric("تۆمارا بینەران (Views)", st.session_state.visits)
        st.write("ئەڤە تنێ بۆ تە دیارە مەتین!")
